...
#include "../Graphics/GraphicsR5G5B4A2.h"
	...
	VGA14BitI::VGA14BitI(const int i2sIndex): VGA(i2sIndex)
	{
		interruptStaticChild = &VGA14BitI::interrupt;
	}

	bool VGA14BitI::init(const Mode &mode, 
		...
		const int hsyncPin, const int vsyncPin, const int clockPin)
	{
		int pinMap[16] = {
	...
	bool VGA14BitI::init(const Mode &mode, const int *redPins, const int *greenPins, const int *bluePins, const int hsyncPin, const int vsyncPin, const int clockPin)
	{
		int pinMap[16];
		for (int i = 0; i < 5; i++)
		{
	...
	bool VGA14BitI::init(const Mode &mode, const PinConfig &pinConfig)
	{
		int pins[16];
	...
	long VGA14BitI::syncBits(bool hSync, bool vSync)
	{
		return ((hSync ? hsyncBit : hsyncBitI) | (vSync ? vsyncBit : vsyncBitI)) * 0x10001;
	}

	int VGA14BitI::bytesPerSample() const
	{
		return 2;
	}

	float VGA14BitI::pixelAspect() const
	{
		return 1;
	}

	void VGA14BitI::propagateResolution(const int xres, const int yres)
	{
		setResolution(xres, yres);
	}

	void VGA14BitI::show(bool vSync)
	{
		if (!frameBufferCount)
			return;
		if (vSync)
